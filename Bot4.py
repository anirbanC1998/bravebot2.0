...
class Bot4:
    ...
    # Bot 4 is similar to Bot3, but it computes a joint probability matrix for the crew members
    ...
    def update_prob_matrices(self, beep_detected, alien_sensed):
        ...
        for _ , crew_pos in enumerate(self.crew_positions):
            if crew_pos is None:
                #By skipping the crew member, it should be able only compute one of them, not both
    ...
    def move_based_on_prob(self):
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx < self.dimension and 0 <= ny < self.dimension and self.grid[nx, ny] != '#' and self.grid[
                ...
                total_crew_sum = self.crew_prob_matrix[nx, ny] - self.alien_prob_matrix[
                    nx, ny]
