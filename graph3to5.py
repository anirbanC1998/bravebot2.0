...
def simulate_bot(bot_class, alpha, k, num_trials):
    ...
    for _ in range(num_trials):
        ...
        success, moves, saved = bot.run()  # Assume .run() returns success, moves, crew saved
        if success:
            success_count += 1
        total_moves += moves
        crew_saved += saved
    ...
    avg_crew_saved = crew_saved / num_trials

    return success_rate, avg_moves, avg_crew_saved
...
def simulate_and_plot(bot_classes, k_values, alpha_range, num_trials):
    for k in k_values:
        ...
        for bot_class in bot_classes:
            ...
            success_rates, avg_moves, avg_crew_saved = zip(*results)  # Unpack results
            ...
            plt.subplot(3, 1, 3)
            plt.plot(alpha_range, avg_crew_saved, label=f'{bot_class.__name__} k={k}')
            plt.xlabel('Alpha')
            plt.ylabel('Average Crew Saved')
            plt.title('Average Crew Saved vs Alpha')

        plt.legend()
        plt.tight_layout()
        plt.savefig(f'{k}Bot345_performance.png', dpi=300)
        plt.show()

if __name__ == "__main__":
    bot_classes = [Bot3, Bot4, Bot5]
    k_values = [1,3,5]  
    alpha_range = np.linspace(0.001, 0.1, 5) 
    num_trials = 10 # Adjust based on your needs
