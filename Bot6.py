...
class Bot6:
    ...
    # Bot 6 is exactly like Bot3, but with 2 crew members, 2 alien members and their own probability matrices
    ...
    def __init__(self, dimension=10, alpha=0.01, k=5):
        ...
        self.alien_prob_matrices = [np.zeros((dimension, dimension)),
                                    np.zeros((dimension, dimension))]
    ...
    def update_prob_matrices_initial(self):
            ...
            for y in range(self.dimension):
                if self.grid[x, y] != '#':  # Check if the cell is not a wall
                    ...
                    if self.distance((x, y), self.bot_pos) > 2 * self.k + 1:
                        ...
                        self.alien_prob_matrices[0][x, y] = 0
                        self.alien_prob_matrices[1][x, y] = 0  # Cells within the k-radius are initially improbable
                        # for the alien
        ...
        self.alien_prob_matrices[0] /= np.sum(self.alien_prob_matrices[0])
        self.alien_prob_matrices[1] /= np.sum(self.alien_prob_matrices[1])
    ...
    def move_based_on_prob(self):
        ...
        for dx, dy in directions:
            ...
            if (0 <= nx < self.dimension and 0 <= ny < self.dimension and self.grid[nx, ny] != '#'
                ...
                total_crew_sum = (sum(matrix[nx, ny] for matrix in self.crew_prob_matrices)
                                  - sum(matrix[nx, ny] for matrix in self.alien_prob_matrices))
