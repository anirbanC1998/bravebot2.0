...
class Bot7:
    ...
    # Bot 7 is exactly like Bot6, but with 2 crew members, 2 alien members and their own probability matrices, with correct joint update
    ...
    def __init__(self, dimension=10, alpha=0.01, k=5):
        ...
        self.alien_positions = [self.place_random(outside_k=True), self.place_random(outside_k=True)]
    ...
    def update_grid(self):
        ...
        for i in range(2):
            if self.crew_positions[i] is None:
    ...
    def update_prob_matrices_initial(self):
        crew_unoccupied_cells = self.dimension ** 2 - len(
            [pos for pos in self.crew_positions if pos is not None]) - 1  # Cells where crew could possibly be, minus 1 for the bot's position
        
        alien_cells = self.dimension**2 - ((2*self.k+1)**2) - 1 #Cells where Alien could be
            ...
            for y in range(self.dimension):
                if self.grid[x, y] != '#':  # Check if the cell is not a wall
                    ...
                    if self.distance((x, y), self.bot_pos) > 2 * self.k + 1:
                        ...
                        # Cells within the k-radius are initially improbable
                        # for the alien
                    # Split the crew probability between the two possible crew member locations
                    # Equally distribute probability among all non-wall cells

        # Normalize each matrix to ensure their probabilities sum to 1, the denominator should never equal 0!
    ...
    def sense_environment(self):
        ...
        for x in range(self.dimension):
            ...
            break
        if beep_detected:  # Assess reliability on uniform random. Now we know that C exists on our path. Beep
            # detected for any crew member
            beep_detected = any(
    ...
    def update_prob_matrices(self, beep_detected, alien_sensed):
        ...
        for _ , crew_pos in enumerate(self.crew_positions):
            ...
            total_prob = np.sum(new_crew_prob_matrix)
            if total_prob > 0:
                self.crew_prob_matrix = new_crew_prob_matrix / total_prob

        if alien_sensed:
            new_alien_prob_matrix = np.zeros_like(self.alien_prob_matrix)
        # Update alien probability matrix using Bayesian updating within the radius
            for dx in range(-2 * self.k, 2 * self.k + 1):
                for dy in range(-2 * self.k, 2 * self.k + 1):
                    x, y = self.bot_pos[0] + dx, self.bot_pos[1] + dy
                    if 0 <= x < self.dimension and 0 <= y < self.dimension and self.grid[x, y] != '#' and alien_sensed:
                        new_alien_prob_matrix[x, y] = self.alien_prob_matrix[x, y] * 4 #2 aliens, so double?
                        # Need to figure if probability differs if we always assume worst case, I'll use 2 for now

            # Normalize the crew probability matrix to ensure probabilities sum to 1
                # Normalize the alien probability matrix
            total_prob = np.sum(new_alien_prob_matrix)
            if total_prob > 0:
                self.alien_prob_matrix = new_alien_prob_matrix / total_prob

    def move_based_on_prob(self):
        directions = [(0, 1), (1, 0), (0, -1), (-1, 0)]  # Up, Right, Down, Left
        best_move = None
        best_crew_score = float('-inf')  # Initialize with the lowest possible score for crew

        for dx, dy in directions:
            nx, ny = self.bot_pos[0] + dx, self.bot_pos[1] + dy
            # Ensure the move is within bounds and not into a wall.
            # The bot can see whats next to it, it will obviously go to crew, and obviously avoid aliens
            if (0 <= nx < self.dimension and 0 <= ny < self.dimension and self.grid[nx, ny] != '#'
                    and self.grid[nx, ny] != 'A'):
                ...
                total_crew_sum = self.crew_prob_matrix[nx, ny] - self.alien_prob_matrix[nx, ny]
    ...
    def move_alien_randomly(self):
        ...
        for i in range(2):
            ...
            for dx, dy in directions:
                new_pos = (self.alien_positions[i][0] + dx, self.alien_positions[i][1] + dy)
                ...
                if 0 <= new_pos[0] < self.dimension and 0 <= new_pos[1] < self.dimension and self.grid[new_pos] != '#':
                    self.alien_positions[i] = new_pos
                    break
                # It should not break out of the outer loop.. please.
           ...
           # print(f"Beep Detected: {beep_detected}, Alien Sensed: {alien_sensed}")
            ...
            for i, crew_pos in enumerate(
                ...
                if crew_pos and self.bot_pos == crew_pos:
                    print(f"Bot 7 rescued crew member at position {crew_pos}")
           ...
           # print(f"Step: {steps}.")
            ...
            for _ , alien_pos in enumerate(
                ...
                if self.bot_pos == alien_pos:
                    print(f"Bot 7 was destroyed by the aliens after {steps + 1} steps.")
