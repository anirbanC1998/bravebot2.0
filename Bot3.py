...
class Bot3:
    ...
    def __init__(self, dimension=10, alpha=0.01, k=5):
        ...
        self.crew_positions = [self.place_random(exclude=[self.bot_pos]), self.place_random(exclude=[self.bot_pos])] #2 crew
    ...
    def update_prob_matrices_initial(self):
        crew_unoccupied_cells = self.dimension**2 - len([pos for pos in self.crew_positions if pos is not None]) - 1  # Minus 1 for the bot's position
    ...
    def sense_environment(self):   
        ...
        for x in range(self.dimension):
            for y in range(self.dimension):
                ...
                if(self.distance(self.bot_pos, (x,y)) == min_distance): #Check for min distance, but the Bot3 does not know which crew it is
        ...
        if beep_detected: #Assess reliability on uniform random. Now we know that C exists on our path. Beep detected for any crew member
            any(random.random() <= np.exp(-self.alpha * (self.distance(self.bot_pos, crew_pos) - 1)) for crew_pos in self.crew_positions if crew_pos is not None)
            ...
            for x in range(self.dimension):
                for y in range(self.dimension):
                    ...
                    if self.distance((x, y), self.bot_pos) <= (2 * self.k + 1):
                        ...
                        new_alien_prob_matrix[x, y] = self.alien_prob_matrix[x, y] * 0.5
    ...
    def move_based_on_prob(self):
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx < self.dimension and 0 <= ny < self.dimension and self.grid[nx, ny] != '#' and self.grid[nx,ny] != 'A':
                ...
                total_crew_sum = sum(matrix[nx, ny] for matrix in self.crew_prob_matrices) - self.alien_prob_matrix[nx, ny]
    ...
    def run(self):
        ...
        while steps <= 10000:
            ...
            for i, crew_pos in enumerate(self.crew_positions): #Need to keep track of C rescued, if all crew_pos is None, every crew is rescued
